...
func (cls *GoClassDefinition) AddMethod(newmthd GoMethod) {
	if cls.program.config.ignoremethod {
		return
	}
...
func (mthd *GoClassMethod) paramList() *ast.FieldList {
	if mthd.method_type == mt_test {
		...
		pfld := makeVar("t", &ast.StarExpr{X: selexp})
	...
	if mthd.params != nil && len(mthd.params) > 0 {
		...
		for i, fp := range mthd.params {
			flist[i] = makeVar(fp.Name(), fp.Type())
		}
...
func (mthd *GoClassMethod) recv() *ast.FieldList {
	...
	rlist[0] = makeVar(mthd.rcvr.GoName(),
...
func (ec *GoEnumConstant) ValueSpec(num int, typeident *ast.Ident) *ast.ValueSpec {
	...
	vals = []ast.Expr{ast.NewIdent(fmt.Sprintf(`"%s"`, ec.name))}

	if typeident == nil {
		return &ast.ValueSpec{Names: []*ast.Ident{ast.NewIdent(fmt.Sprintf("%s_%s\t%s", ec.name, ec.enumType, ast.NewIdent(ec.enumType)))},
			Values: vals}
	}

	return &ast.ValueSpec{Names: []*ast.Ident{ast.NewIdent(fmt.Sprintf("%s_%s", ec.name, ec.enumType))},
...
func (gp *GoProgram) analyzeEnumConstant(name string, con *grammar.JEnumConstant) *GoEnumConstant {
	...
	return &GoEnumConstant{name: con.Name, enumType: name}
}
